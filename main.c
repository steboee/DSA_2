...
TABLE* table;
int max=10000000;                   // počitočná veľkosť hashovacej tabulky
...
int main(){
    int items = 50000;                                // pocet dát ktoré chcem vložiť do hashovacej tabulky
    ...
    for (int i =0;i<items;i++){                         // TESTOVAC , vkladá (items) dát do tabulky
        ...
        clock_t stop = clock();
        elapsed = elapsed + (double)(stop - start) / (double)CLOCKS_PER_SEC;
    }
    ...
    for (int i =0;i<items;i++){                         // TESTOVAC , vkladá (items) dát do tabulky
        ...
        clock_t stop = clock();
        elapsed2 = elapsed2 + (double)(stop - start) / (double)CLOCKS_PER_SEC;
    }
